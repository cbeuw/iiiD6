...
impl Sampler {
    ...
    fn discover_R_bound(grid_size: isize, orbital: &orbital::Orbital, sample_amount: usize) -> Result<f64, &'static str> {
        ...
        let row_bound:f64;
        let col_bound:f64;
        if let Some((row,_)) = row_cum.iter().map(|&x| x / grid_size as f64).enumerate().find(|&(_,prob)| prob >= PROB_THRESHOLD_AT_R_MAX) {
        ...
        if let Some((col,_)) = col_cum.iter().map(|&x| x / grid_size as f64).enumerate().find(|&(_,prob)| prob >= PROB_THRESHOLD_AT_R_MAX) {
        ...
        Ok(if row_bound > col_bound {row_bound} else {col_bound})
